...
void	unset_export_2(char **words, t_ls *data, t_v **v, t_v *t)
{
	data->c_e[1] = 'a';
	t = *v;
...
void	unset_export_3(char **words, t_ls *data, t_v **v, t_v *t)
{
	data->c_e[1] = 'a';
	t = *v;
...
void	unset_export(char **words, t_ls *data, t_v **v, char *temp)
{
	...
	if (ft_strcmp_2(tmp, temp, 1) == 0
		...
		unset_export_2(words, data, v, t);
	else
		unset_export_3(words, data, v, t);
	free(tmp);
	free(temp);
	data->c_e[count] = '\0';
}
